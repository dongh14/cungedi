...
### Browse Experience
- 已保存地点会显示在应用内列表中。
- 有坐标或保守近似坐标的已保存地点会显示在简单地图中。
- 地图支持基本缩放。
- 用户可以按城市查看地点。
- 没有坐标的地点仍然必须在列表里清晰可用。
- 当地图使用近似城市级或区域级坐标时，界面必须明确标记为近似位置。

### Location Handling
- 如果系统可以确定地点位置，应尝试为地点补全坐标。
- 如果无法获得精确坐标，地点仍然可以保存并继续显示在列表中。
- 即使没有坐标，城市仍然是必填字段。
- V1 地图栈应使用开源方案：MapLibre GL JS、OpenStreetMap 衍生数据，以及自托管的 Protomaps PMTiles 底图。
- V1 不使用 Google Maps、Apple Maps、高德、Mapbox 或其他付费/计量地图 API 作为地图渲染基础。
- V1 不应使用 OpenStreetMap 公共瓦片服务器作为生产瓦片后端。
- 当无法获得精确坐标时，系统可以使用保守的城市级或区域级近似坐标帮助地图浏览，但不能伪装成精确 POI 坐标。
- 当位置是近似的，UI 必须明确标记，不得暗示导航级精度。
- V1 不自托管 Nominatim，也不引入付费地理编码提供商。
...
- 系统必须允许在没有确认坐标时保存地点。
- 系统必须使用 MapLibre GL JS 进行 V1 地图渲染。
- 系统必须使用基于 OpenStreetMap 衍生数据的自托管 PMTiles 底图，而不是生产环境下的公共 OSM 瓦片服务。
- 系统必须支持高德地图链接和分享文案作为官方 V1 来源。
- 系统应把大众点评视为重要但 best-effort 的来源。
- 系统应把小红书和抖音视为 best-effort 来源。
- 系统可以接受百度地图作为次要、best-effort 输入来源。
- 系统可以支持 Google Maps 作为海外可选来源，但它不是大陆中国 V1 的核心承诺。
- 系统必须展示已保存地点的列表视图。
- 系统必须展示有坐标地点的地图视图。
- 系统可以在精确坐标缺失时展示明确标记为近似的城市级或区域级地图点位。
...
- 仅依赖开源地图底图与保守近似坐标意味着地图更偏浏览用途，而不是精确导航用途。
- 城市级或区域级近似坐标如果标记不清，会让用户误以为点位精确。
